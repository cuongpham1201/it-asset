...
const viToEn: Array<[string,string]> = [
  ...
  ['Quản trị các danh mục dùng chung trong toàn bộ quy trình tài sản.','Manage shared master data across the asset lifecycle.'],
  ['Kết nối Microsoft 365/Entra ID hoặc LDAP/Active Directory để đồng bộ hồ sơ, phòng ban và phạm vi phân quyền.','Connect Microsoft 365/Entra ID or LDAP/Active Directory to synchronize users, departments and access scope.'],
  ['Thiết lập ngôn ngữ mặc định, múi giờ và cách hiển thị ngày giờ trên toàn hệ thống.','Configure the default language, timezone and date/time display across the system.'],
  ['Thay đổi logo, tên phần mềm và thông tin công ty trên toàn hệ thống.','Change the logo, application name and company information across the system.'],
  ['Thiết lập thông tin sử dụng khi gửi biên bản bàn giao tài sản.','Configure information used when sending asset handover records.'],
  ...
  ['Quét mã, khai báo trực tiếp hoặc import danh sách tài sản từ Excel.','Scan a code, enter details directly, or import an asset list from Excel.'],
  ['Quét mã vào phiếu nhập','Scan a code into the receipt'],
  ['Camera, máy quét USB hoặc nhập mã thủ công. Mã chỉ dùng để nhận dạng và kiểm tra trùng.','Use a camera, USB scanner, or manual entry. The code is only used for identification and duplicate checking.'],
  ...
  ['Nhập kho luôn tạo tài sản ở trạng thái Sẵn sàng và chưa gán người sử dụng.','Receiving always creates an Available asset with no assigned user.'],
  ...
  ['Tra cứu tài sản đã có hoặc quét mã để lập đầy đủ hồ sơ tài sản nhập kho.','Look up an existing asset or scan a code to create a complete receipt record.'],
  ['Tra cứu tài sản','Asset lookup'],
  ['Nhập kho tài sản mới','Receive new asset'],
  ['Quét tài sản','Scan asset'],
  ['Đưa barcode vào camera hoặc đặt con trỏ vào ô dưới và dùng máy quét USB.','Place the barcode in front of the camera, or focus the field below and use a USB scanner.'],
  ['Dữ liệu scan lấy từ đâu?','Where does scanned data come from?'],
  ['Camera đọc hình barcode/QR; máy quét USB gõ chuỗi mã vào ô; hệ thống dùng chuỗi đó để đối chiếu mã tài sản hoặc serial trong sổ tài sản.','The camera reads a barcode/QR image; a USB scanner enters the code; the system matches it against the asset code or serial number in the asset register.'],
  ...
  ['Các thay đổi được áp dụng cho tài khoản hiện tại và dùng làm mặc định khi tạo người dùng mới.','Changes apply to the current account and become the default for new users.'],
  ['Thời điểm nghiệp vụ vẫn được lưu theo UTC trong cơ sở dữ liệu. Cài đặt này chỉ thay đổi cách hiển thị, không làm thay đổi dữ liệu audit.','Business timestamps remain stored as UTC in the database. This setting only changes display and does not alter audit data.'],
  ...
  ['Client secret và mật khẩu bind không được trả về trình duyệt sau khi lưu. Bản production phải giữ chúng trong secret store phía server.','Client secrets and bind passwords are never returned to the browser after saving. Production must keep them in a server-side secret store.'],
  ['Sử dụng ứng dụng Entra ID với quyền Microsoft Graph User.Read.All và GroupMember.Read.All.','Use an Entra ID application with Microsoft Graph User.Read.All and GroupMember.Read.All permissions.'],
  ['Tài khoản dịch vụ chỉ cần quyền đọc OU và thuộc tính người dùng được chọn.','The service account only needs read access to the selected OU and user attributes.'],
  ...
  ['Đồng bộ cả tài khoản đã vô hiệu hóa để phục vụ lịch sử kiểm toán','Synchronize disabled accounts for audit history'],
  ...
  ['Đã lưu cấu hình không nhạy cảm. Secret/mật khẩu không được lưu trong trình duyệt và phải chuyển tới secret store qua backend.','Non-sensitive settings saved. Secrets/passwords are not stored in the browser and must be sent to the server-side secret store.'],
  ['Cấu hình bắt buộc đã đầy đủ. Backend sẽ thực hiện kết nối thật và xác minh quyền đọc người dùng.','Required settings are complete. The backend will perform the connection and verify user read permissions.'],
  ['Chưa đủ thông tin bắt buộc để kiểm tra kết nối.','Required information is missing for the connection test.'],
  ['Đang bật','Enabled'],['Đang tắt','Disabled'],
  ['Thông tin nhận diện','Brand identity'],
  ['Thay đổi logo, thông tin công ty và mẫu biên bản bàn giao A4.','Change the logo, company information and A4 handover template.'],
  ['Các trường công ty và bộ phận sẽ tự động xuất hiện trên biên bản bàn giao.','Company and department fields appear automatically on the handover record.'],
  ...
  ['Bản frontend mở ứng dụng email mặc định với nội dung được soạn sẵn. Gửi tự động sẽ được kích hoạt khi kết nối API email ở backend.','The frontend opens the default email application with prepared content. Automatic sending will be enabled after connecting the backend email API.'],
...
function translateText(value:string,language:string){
  ...
  const countPatterns:Array<[RegExp,(match:RegExpMatchArray)=>string]>=[
    ...
    [/^(\d+) ngôn ngữ phổ biến được hỗ trợ\. Thay đổi ngôn ngữ được áp dụng ngay\.$/,match=>`${match[1]} popular languages are available. Language changes are applied immediately.`],
...
function translateRoot(root:ParentNode,language:string){
  ...
  root.querySelectorAll<HTMLOptionElement>('option:not([value])').forEach(option=>option.setAttribute('value',option.value))
...
export function useRuntimeI18n(language:string){
  useEffect(()=>{
    let scheduled=false
    const apply=()=>{scheduled=false;translateRoot(document.body,language)}
