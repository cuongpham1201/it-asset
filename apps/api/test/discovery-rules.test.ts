...
test('fingerprint matches the Agent normalization contract',()=>{
  ...
  const reordered:any={device:{...payload.device,network_interfaces:[{mac_address:'aa'},{mac_address:'bb'}]}}
...
test('matching never resolves ambiguous candidates automatically',()=>{
  ...
  assert.deepEqual(classifyCandidates([{id:'asset-1',systemUuid:null,serialNumber:'SN-01',macAddress:null}],{systemUuid:null,serial:'sn-01',mac:null}),{status:'MATCHED',suggestedAssetId:'asset-1',confidence:95,reason:null})
  assert.equal(classifyCandidates([{id:'a',systemUuid:null,serialNumber:null,macAddress:'AA'},{id:'b',systemUuid:null,serialNumber:null,macAddress:'AA'}],{systemUuid:null,serial:null,mac:'AA'}).status,'CONFLICT')
})
