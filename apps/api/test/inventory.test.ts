...
test('closing an inventory marks pending items missing and writes immutable asset history',async()=>{
  let missingFilter:any,historyRows:any[]=[]
  const tx={
    inventorySession:{findUnique:async()=>({id:'session-1',inventoryNo:'KK-001',status:'OPEN',scopeDepartmentId:null,items:[{assetId:'asset-1'},{assetId:'asset-2'}]}),update:async()=>({id:'session-1',status:'CLOSED'})},
    inventoryItem:{updateMany:async(args:any)=>{missingFilter=args;return {count:2}}},
    assetHistory:{createMany:async({data}:any)=>{historyRows=data;return {count:data.length}}},
