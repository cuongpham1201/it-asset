...
import { assertTypeFields,availableQuantity,daysUntil,entitlementStatus } from '../src/modules/renewals/renewals.rules'
...
test('renewal email recipients are normalized and deduplicated',()=>{
  assert.deepEqual(notificationRecipients(['IT@company.vn',' it@company.vn '],true,'owner@company.vn'),['it@company.vn','owner@company.vn'])
