...
import { IsBoolean,IsDateString,IsEnum,IsIn,IsInt,IsNotEmpty,IsOptional,IsString,IsUUID,Max,MaxLength,Min } from 'class-validator'
...
export class ListIncidentsQuery{
  ...
  @IsOptional() @IsIn(['all','open','critical','resolved','overdue','downtime']) view:'all'|'open'|'critical'|'resolved'|'overdue'|'downtime'='all'
