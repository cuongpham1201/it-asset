...
import { ArrayMaxSize,IsArray,IsDateString,IsIn,IsInt,IsNotEmpty,IsOptional,IsString,IsUUID,Length,Max,MaxLength,Min,ValidateNested } from 'class-validator'
...
export class NetworkInterfaceDto{
  ...
  @IsOptional() @IsArray() @ArrayMaxSize(64) @IsString({each:true}) @MaxLength(100,{each:true}) addresses?:string[]
}
export class AgentDeviceDto{
  ...
  @IsArray() @ArrayMaxSize(128) @ValidateNested({each:true}) @Type(()=>NetworkInterfaceDto) network_interfaces!:NetworkInterfaceDto[]
}
