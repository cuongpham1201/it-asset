...
export const daysUntil=(date:Date,now=new Date())=>Math.ceil((new Date(date.getFullYear(),date.getMonth(),date.getDate()).getTime()-new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime())/86400000)
export const entitlementStatus=(expiryDate:Date,now=new Date())=>daysUntil(expiryDate,now)<0?'EXPIRED':daysUntil(expiryDate,now)<=30?'EXPIRING':'ACTIVE'
