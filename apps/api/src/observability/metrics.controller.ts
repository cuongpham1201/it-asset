...
export class MetricsController{
  ...
  async read(@Headers('authorization') authorization?:string){const token=process.env.METRICS_TOKEN;if(process.env.NODE_ENV==='production'&&!token)throw new ServiceUnavailableException('METRICS_TOKEN is required in production');if(token&&authorization!==`Bearer ${token}`)throw new UnauthorizedException();let ready=true;try{await this.db.$queryRaw`SELECT 1`}catch{ready=false}return this.metrics.render(ready)}
}
