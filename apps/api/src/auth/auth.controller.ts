...
export class AuthController{
  ...
  @Get('me')
  me(@Req() request:Request&{authUser:any}){return {user:this.auth.toClientUser(request.authUser)}}
  ...
  async changePassword(@Body() body:ChangePasswordDto,@Req() request:Request&{authUser:any},@Res({passthrough:true}) response:Response){
    await this.auth.changePassword(request.authUser.id,body.newPassword)
    response.clearCookie(COOKIE,cookieOptions)
  }

  @Post('logout')
  @HttpCode(204)
  async logout(@Req() request:Request&{sessionToken?:string},@Res({passthrough:true}) response:Response){
